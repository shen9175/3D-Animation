...
bool M3DLoader::LoadM3d(const std::string& filename, 
	...
	if( fin )
	{
		...
		ReadMaterials(fin, numMaterials, mats);
		ReadSubsetTable(fin, numMaterials, subsets);
		ReadSkinnedVertices(fin, numVertices, vertices, vertextype);
		ReadTriangles(fin, numTriangles, indices);
...
void M3DLoader::ReadMaterials(std::ifstream& fin, UINT numMaterials, std::vector<M3dMaterial>& mats)
{
	...
	// std::string normalMapName;
	 ...
	 for(UINT i = 0; i < numMaterials; ++i)
	 {
			...
			fin >> ignore >> mats[i].DiffuseMapName;//diffuseMapName;
			fin >> ignore >> mats[i].NormalMapName;//normalMapName;
...
/*
void M3DLoader::ReadVertices(std::ifstream& fin, UINT numVertices, std::vector<PosNormalTex>& vertices)
{
	std::string ignore;
...
void M3DLoader::ReadVertices(std::ifstream& fin, UINT numVertices, std::vector<PosNormalTexTan>& vertices)
{
	std::string ignore;
    vertices.resize(numVertices);
...
*/
//void M3DLoader::ReadSkinnedVertices(std::ifstream& fin, UINT numVertices, std::vector<PosNormalTexTanAnimation>& vertices)
void M3DLoader::ReadSkinnedVertices(std::ifstream& fin, UINT numVertices, void*& vertices, std::string vertextype)
{
	//std::vector<PosNormalTexTanAnimation> ppp;
	std::string ignore;
	if (vertextype == "PosNormalTex") {
		PosNormalTex* temp = new PosNormalTex[numVertices];
		fin >> ignore; // vertices header text
		for(UINT i = 0; i < numVertices; ++i)
		{
			fin >> ignore >> temp[i].Pos.x     >> temp[i].Pos.y      >> temp[i].Pos.z;
			fin >> ignore >> temp[i].Normal.x  >> temp[i].Normal.y   >> temp[i].Normal.z;
			fin >> ignore >> temp[i].Tex.x     >> temp[i].Tex.y;
		}
		vertices = static_cast<void*>(temp);
	} else if (vertextype == "PosNormalTexTan") {
		PosNormalTexTan* temp = new PosNormalTexTan[numVertices];
		fin >> ignore; // vertices header text
		for(UINT i = 0; i < numVertices; ++i)
		{
			fin >> ignore >> temp[i].Pos.x     >> temp[i].Pos.y      >> temp[i].Pos.z;
			fin >> ignore >> temp[i].TangentU.x>> temp[i].TangentU.y >> temp[i].TangentU.z >> temp[i].TangentU.w;
			fin >> ignore >> temp[i].Normal.x  >> temp[i].Normal.y   >> temp[i].Normal.z;
			fin >> ignore >> temp[i].Tex.x     >> temp[i].Tex.y;
		}
		vertices = static_cast<void*>(temp);
	} else if (vertextype == "PosNormalTexTanAnimation") {
		PosNormalTexTanAnimation* temp = new PosNormalTexTanAnimation[numVertices];
		fin >> ignore; // vertices header text
		int boneIndices[4];
		float weights[4];
		for (UINT i = 0; i < numVertices; ++i) {
...
//			}
			...
			temp[i].Weights.x = weights[0];
			temp[i].Weights.y = weights[1];
			temp[i].Weights.z = weights[2];

			temp[i].BoneIndices[0] = (BYTE)boneIndices[0];
			temp[i].BoneIndices[1] = (BYTE)boneIndices[1];
			temp[i].BoneIndices[2] = (BYTE)boneIndices[2];
			temp[i].BoneIndices[3] = (BYTE)boneIndices[3];
			//ppp.push_back(temp[i]);
		}
		vertices = static_cast<void*>(temp);
	}
}
